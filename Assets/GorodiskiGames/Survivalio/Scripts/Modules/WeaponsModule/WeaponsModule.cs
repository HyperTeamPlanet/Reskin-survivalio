...
namespace Game.Modules
{
    public sealed class WeaponsModule : Module<WeaponsModuleView>
    {
        ...
        private void OnWeaponReady(WeaponController weapon)
        {
            ...
            var multipleBulletsDelay = weapon.Model.MultipleBulletsDelay;
            _view.StartCoroutine(SpawnBulletsCoroutine(weapon, multipleBulletsDelay));
        ...
        private Vector3 GetAimPosition(UnitController unit, float range, BulletControllerType bulletType)
        {
            ...
            var isNeedUnitToAim = bulletType == BulletControllerType.Straight || bulletType == BulletControllerType.Return;
            if(isNeedUnitToAim)
            {
